...
x days", where $x \in \{3,5,10,15,20..\}$.

The resulting covariate matrix is roughly 3162 by 193 at the state level, and OOO
by OOO at the county level.
...
The numbers in the county level data are.. (todo: either fill these in or exclude altogether)
...
This depends a bit on how the training/test split is made among geographical
regions. If we use a different random seed for splitting, the ROC curves can
differ quite a bit:

![](figures/for_v1_report/ROC_state_newseed.png)
...
```{r form-data, eval=FALSE}
## Setup
lags = 28
geo_type = "state" ## or "county", or "MSA"
...
df_traintest %>% select(resp) %>% table() %>% print()
splitted$df_train %>% select(resp) %>% table() %>% print()
splitted$df_test %>% select(resp) %>% table() %>% print()
df_validation %>% select(resp) %>% table() %>% print()
foldid <- make_foldid(splitted$df_train, nfold = 5)
...
Also visualize the 1's and 0's in a plot, in the state level data.

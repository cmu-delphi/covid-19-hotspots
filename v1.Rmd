...
df_model <- ready_to_model(mat, lags, n_ahead, response, slope, fn_response, threshold, TRUE)
df_model <- ready_to_model(mat, lags, n_ahead, response, slope, fn_response, threshold, FALSE)
df_validation <- df_model %>% filter(time_value %in% validation_days)
df_validation %>% select(resp) %>% unlist() %>% table()
